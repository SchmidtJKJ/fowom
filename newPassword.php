...
{
    header("Location: konekte.php?cannot%access=0&change!password&that%way");
    exit();
  }
  ...
  if(isset($_POST["valide"]))
  {
      if(isset($_POST['psswd2']) &&isset($_POST['psswd1']))
      {
        ...
        $psswd =htmlentities($_POST['psswd1']);
        $psswd2 =htmlentities($_POST['psswd2']);
        $email = $_SESSION['username'];
        $kod = htmlentities($_POST['idNumber']);

        if ($_POST['psswd1'] != $_POST['psswd2'])
          {
            $error = "Modpas yo pa menm! Eseye ankò!";
          }


          $token = "SELECT* from verifikasyon JOIN itilizate on verifikasyon.met = itilizate.userID WHERE itilizate.imel ='$email';";
          ...
          if ($kod!=$token)
          {
              $error ="Dezole, kòd ou rantre a pa bon!";
          }

        else {
                  $hashedPWD= password_hash($psswd, PASSWORD_DEFAULT);
                  $sql = "UPDATE itilizate SET modpas =? WHERE imel =?";
                  $query = mysqli_stmt_init($conn);
                  if (!mysqli_stmt_prepare($query, $sql))
                  {
                    header("Location: konekte.php?SQLerror");
                    exit();
                  }
                  else {
                    mysqli_stmt_bind_param($query,"ss",$hashedPWD, $email);
...
<link rel="shortcut icon" href="images/GolAyiti-logo.jpg" />
   ...
   <section class="pt-0 bg-primary">
 			...
 			<div class="row">
 				...
 				<div class="col-md-12 col-lg-12">
 					...
 					<div class="ptb-0">
 						...
 						<a class="mt-10"><i class="mlr-10 ion-chevron-right"></i><b>NOUVO MODPAS</b></a>
 	...
 	<section class="pt-0 pb-20">
 		...
 		<div class="container">
          ...
          <?php if($sikse) {?>
              <h3> <em>Modpas ou a chanje ak siksè! Ou ka itlizel depi kounya pou ou konekte</em></h3>
              <br>
    					<a class="btn-fill-red plr-10 mtb-10 btn-b-md" href="konekte.php"><b>KONEKTE</b></a>
            <?php }
            else{ ?>
           <form action="" method="POST">
 					<div class="p-30 mb-30 card-view">

 						<h4 class="p-title"><b>CHANJMAN</b></h4>

 							<div class="row">
 								<div class="col-sm-6">
                   <label for="">Antre kòd ou te resevwa nan imèl ou a</label>
 									<input class="mb-30" type="text"  name="idNumber" required>
 									<div class="red-text">
                 ...
                 <br> &nbsp;
 								<div class="col-sm-6">
 									<input class="mb-30" type="password" placeholder="antre nouvo modpas la" name="pass1" required>
 								</div><!-- col-sm-6 -->
                 <br>
                 <br> &nbsp;
 								<div class="col-sm-6">
 									<input class="mb-30" type="password" placeholder="re antre nouvo modpas la" name="pass2" required>
 								</div><!-- col-sm-6 -->
                 <br>


 							</div><!-- row -->
 							<button class="btn-fill-primary plr-20" type="submit" name="valide">Valide</button>
 ...
 <!-- This going to be google api maps credentials -->
